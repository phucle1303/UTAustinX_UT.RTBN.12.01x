...
// **write this function**
  enum DRESULT retVal = RES_OK;
  uint16_t i;
  uint32_t sectorAddress = EDISK_ADDR_MIN + 512*sector;
  uint8_t *ptFlash = (uint8_t *)(sectorAddress);
  if (sectorAddress > EDISK_ADDR_MAX)
  {
    retVal = RES_PARERR;
  }
  else
...
// **write this function**
  ...
  {
    Flash_WriteArray((uint32_t *)buff, sectorAddress, 128u);
  }
  return retVal;
}
